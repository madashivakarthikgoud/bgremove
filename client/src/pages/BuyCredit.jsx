...
const BuyCredit = () => {
  ...
  const paymentRazorpay = async (planId) =>{
    try{
      ...
      const token = await getToken()
      const {data} = await axios.post(backendUrl+'/api/user/apy-razor',{planId},{headers:{token}})
      if(data.success){
        initPay(data.order)
      }
    }catch(error){
  ...
  return (
    <div className="min-h-[80vh] text-center pt-14 mb-10">
      ...
      <h1 className="text-center text-2xl md:text-3xl lg:text-4xl mt-4 font-semibold bg-gradient-to-r from-gray-900 to-gray-400 bg-clip-text text-transparent hover:bg-gradient-to-l mb-6 sm:mb-10" >Choose the plan that's right for you</h1>
